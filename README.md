...
在 BOSS 直聘**公司简介页**、**招聘职位页**增加「拉黑该公司」按钮，点击后自动调用 BOSS 官方接口，将当前公司加入你的账号黑名单，无需再去一个一个的手动输入公司名。

## 功能说明

- 在公司简介页、招聘职位页显示操作按钮（位于「收藏」按钮旁）
  - 公司简介：`/gongsi/{id}.html`
  - 招聘职位：`/gongsi/job/{id}.html`
- **拉黑该公司**：将当前公司加入黑名单；成功后按钮变为「已拉黑」
- **移出黑名单**：仅在「已拉黑」状态下可点击，将公司从屏蔽列表移除
- 自动读取公司名称（工商全称、页面标题、Logo 等；招聘职位页无工商信息时通过 URL 公司 ID 兜底）
...
2. 打开任意公司简介页或招聘职位页，例如：
   - 公司简介：`https://www.zhipin.com/gongsi/xxxxx.html`
   - 招聘职位：`https://www.zhipin.com/gongsi/job/xxxxx.html`
3. 点击 **拉黑该公司** 或 **移出黑名单**（已拉黑时可用）
...
| 点击无反应            | 扩展未加载或不在支持页面    | 检查扩展是否启用，确认 URL 为公司简介或招聘职位页 |
